<?php
/**
 * Template Name: FAQs Search page
 *
 * @package Liveglam
 */
...
$search_faqs = [];
if (isset($_GET['p_type']) && isset($_GET['keyword'])) {
  $p_type = $_GET['p_type'];
  $keyword = $_GET['keyword'];

  $search_faqs = get_posts(array(
    's' => $keyword,
    'post_type' => $p_type,
    'post_status' => 'publish',
    'numberposts' => -1
...
<section class="faq-search-results">
  <div class="container">
      ...
      <?php if (sizeof($search_faqs)) {
          show_faq('search', $search_faqs, 'desktop');
      } ?>
  </div>
</section>
...
function show_faq($category, $faq_data, $device){
  ...
  if(!$category || !in_array($category, array('kissme', 'morpheme', 'shadowme', 'general', 'shipping', 'orders', 'billing', 'rewards', 'dashboard', 'search')))
    return;
    ...
    foreach($faq_data as $faq): $count++;
      echo '<div class="panel">
                <div class="panel-heading">
                    <p class="panel-title">
                        <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion-'.$category.'" href="#accordion-'.$category.'-panel'.$count.'-'.$device.'">
                            <span class="panel-icon panel-open-icon">+</span>
                            <span class="panel-icon panel-close-icon">-</span>'
        .$faq->post_title.'
                        </a>
                    </p>
                </div>
                <div id="accordion-'.$category.'-panel'.$count.'-'.$device.'" class="panel-collapse collapse">
                    <div class="panel-body">'.wpautop($faq->post_content).'</div>
                </div>
            </div>';
    endforeach;
  endif;
