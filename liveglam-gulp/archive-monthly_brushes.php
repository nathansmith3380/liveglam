<?php
  ...
  if(!is_user_logged_in() || !in_array($subscriptions_status['get_status_morpheme'], array('active', 'waitlist'))){
...
<div class="dashboard-content">
  ...
  <?php $count_brushes = 0; $list_monthPost = array();
    while(have_posts()) : the_post();
      ...
      $product_items = array();
      if( !empty( $total_products = get_post_meta($postID, 'add_brushes_for_the_month', true) ) ){
        for($i = 0; $i < $total_products; $i++){
          $product_items[] = array(
            'image' => wp_get_attachment_image_url(get_post_meta($postID, 'add_brushes_for_the_month_'.$i.'_brush_image', true), 'full'),
            'type' => get_post_meta($postID, 'add_brushes_for_the_month_'.$i.'_type', true),
            'title' => get_post_meta($postID, 'add_brushes_for_the_month_'.$i.'_morphe_brush_title', true),
            'desc' => get_post_meta($postID, 'add_brushes_for_the_month_'.$i.'_brushes_description', true),
            'retail' => get_post_meta($postID, 'add_brushes_for_the_month_'.$i.'_retail', true)
          );
        }
      }
        ...
        <div class="wrap">
          <div class="item-content-bg">
            ...
            <div class="tutor" id="br_tutor_<?php echo $key; ?>">
              ...
              <?php } ?>
              <?php echo do_shortcode("[show_section_blogs post_id=".$monthPost['postID']." product=morpheme]"); ?>
            </div>
          </div>
        </div>
      </div>
