<?php
  ...
  if(!is_user_logged_in() || !in_array($subscriptions_status['get_status_kissme'], array('active', 'waitlist'))){
  ...
  <div class="dashboard-content">
    ...
    <?php $count_brushes = 0; $list_monthPost = array();
      while(have_posts()) : the_post();
        $postID = get_the_ID();
        $banner_image_mobile = get_post_meta($postID, 'banner_image_mobile', true)?wp_get_attachment_image_url(get_post_meta($postID, 'banner_image_mobile', true), 'full'):get_stylesheet_directory_uri().'/assets/img/kmdt-banner.jpg';
        $banner_image_desktop = get_post_meta($postID, 'banner_image_desktop', true)?wp_get_attachment_image_url(get_post_meta($postID, 'banner_image_desktop', true), 'full'):get_stylesheet_directory_uri().'/assets/img/placeholder-km-monthly.jpg';

        $product_items = array();
        if( !empty( $total_products = get_post_meta($postID, 'add_lipstick_for_the_month', true) ) ){
          for($i = 0; $i < $total_products; $i++){
            $product_items[] = array(
              'image' => wp_get_attachment_image_url(get_post_meta($postID, 'add_lipstick_for_the_month_'.$i.'_lipstick_image', true), 'full'),
              'type' => get_post_meta($postID, 'add_lipstick_for_the_month_'.$i.'_type', true),
              'title' => get_post_meta($postID, 'add_lipstick_for_the_month_'.$i.'_lipstick_title', true),
              'desc' => get_post_meta($postID, 'add_lipstick_for_the_month_'.$i.'_lipstick_description', true),
              'retail' => get_post_meta($postID, 'add_lipstick_for_the_month_'.$i.'_retail', true)
            );
          }
        }
          ...
          <div class="wrap">
            <div class="item-content-bg">
                        ...
                        <div class="inner new-inner show-mobile">
                          <div class="inner-top">
                            <div class="inner-top-left">
                              <p class="inner-type-retails"><?php echo $item['type']; ?> | Retail: <?php echo $item['retail']; ?></p>
              ...
              <div class="tutor" id="br_tutor_<?php echo $key; ?>">
                ...
                <?php } ?>
                <?php echo do_shortcode("[show_section_blogs post_id=".$monthPost['postID']." product=kissme]"); ?>
              </div>
            </div>
          </div>
        </div>
